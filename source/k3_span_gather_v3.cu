// K9 `k3_span_gather` v3 -- v2 with a bf16 partial input.
//
// Same contract, layout, math and landing points as v2
// (docs/k3-kernel-abi.md section K9); the only difference is the dtype of the
// first argument `partial`.  v2 reads the f32 partial row and rounds each
// element through bf16 before the conv (`k9_land4`), i.e. x_i =
// f32(bf16(partial[i, c])).  v3 takes the partial already landed to bf16 by
// its producer, so the load is a bf16 unpack instead of a float4 round: the
// values entering the conv are identical (the rounding happens in the GEMM
// epilogue instead of here).
//
//   extern "C" __global__ void kern_k3_span_gather(
//       const bf16* partial,       // [rows, KDA_FUSED]  rows at..at+span read
//       ... unchanged from v2 ...
...
// K9_VEC consecutive columns of one partial row (K9_VEC/4 bf16x4 loads).
__device__ __forceinline__ void k9_load_partial(const bf16* __restrict__ p, float* x) {
#pragma unroll
  for (int q = 0; q < K9_VEC / 4; ++q) k9_unpack4(__ldg((const uint2*)(p + 4 * q)), x + 4 * q);
}
...
__device__ __forceinline__ void k9_input(const bf16* __restrict__ col, const bf16* win, int c, int i, float* x) {
  if (i < 0) {
...
template <bool FIRST>
__device__ __forceinline__ void k9_stream(const bf16* __restrict__ partial, const float* __restrict__ cw,
...
#pragma unroll
  ...
  const bf16* __restrict__ col = partial + (size_t)s * K9_INNER + c;
