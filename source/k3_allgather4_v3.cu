...
//
// (This is `source/k3_allgather4_v2.cu` under the name the composed commit
// carries: main still runs `k3_allgather4`, the version whose arrival fences in
// every one of the block's 1024 threads, and this module is what
// `scripts/gen_peer_path.py` points `l*.gather_small` at.  The kernel itself is
// unchanged -- one `red.add.release.sys` per block instead of one
// `__threadfence_system()` per thread, published after the CTA barrier that
// already orders the block's stores.)
//
...
//
//   nvcc -cubin -arch=sm_103a -o k3_allgather4_v2.cubin k3_allgather4_v2.cu
    ...
    int slot, int rank, int nranks)
{
    ...
    /* The arrival is a *release* reduction.  It used to be a
     * __threadfence_system() in each of the block's 1024 threads -- 917504
     * system fences and, worse, 917504 CCTL.IVALL that throw away the L1 of
     * whichever SM runs them, which the *next* kernels pay for.  The CTA barrier
     * already orders every thread's stores before what follows it, and PTX
     * cumulativity makes a release cover everything that happens-before it, so
     * one release reduction per block publishes the same ordering: measured,
     * l*.gather_small 101.2 -> 93.1 us per call against 86.1 us with no ordering
     * at all (which is the push, 77.9, plus the arrival and the wait). */
    __syncthreads();
    if (threadIdx.x == 0)
        asm volatile("red.add.release.sys.global.u64 [%0], %1;"
                     :: "l"(flags + slot), "l"(1ull) : "memory");
