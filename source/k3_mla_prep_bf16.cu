// K4 `k3_mla_prep` v2 with the fused partial already landed to bf16.
//
// Identical to source/k3_mla_prep_v2.cu (`kern_k3_mla_prep_gate`,
// `kern_k3_mla_prep_head`, same geometry, same reduction order, same landing
// points) except that `partial` is `const bf16*` instead of `const float*`:
// v2 loads four f32 and rounds each one to bf16 with `landf` before any
// arithmetic, so when its producer (`l*.wfu`) lands bf16 the round trips are
// no-ops and every value entering the arithmetic is the same number.  The
// loads become 8-byte uint2 (four bf16) instead of 16-byte float4.
//
// Written as a separate file so that a change to k3_mla_prep_v2.cu by its owner
// (agent a) cannot collide with this module in the composed manifest.
//
...
// The partial is already bf16, so every "land to bf16" is the identity.
__device__ __forceinline__ float landf(float x) { return x; }

// four bf16 -> float4 (the shape v2 loads from its f32 partial)
__device__ __forceinline__ float4 load4bf16(const bf16* p) {
...
extern "C" __global__ __launch_bounds__(PG_THREADS) void kern_k3_mla_prep_gate(
    const bf16* __restrict__ partial, bf16* __restrict__ mla_gate, int B) {
  const int c = PG_COLS * threadIdx.x;
  const int r0 = blockIdx.x * PG_ROWS;
  const bf16* src[PG_ROWS];
  bf16* dst[PG_ROWS];
...
#pragma unroll
  for (int r = 0; r < PG_ROWS; ++r) {
    ...
    if (ok) {
      v0[r] = load4bf16(src[r]);
      v1[r] = load4bf16(src[r] + 4);
    }
  }
#pragma unroll
  for (int r = 0; r < PG_ROWS; ++r) {
...
extern "C" __global__ __launch_bounds__(PH_THREADS) void kern_k3_mla_prep_head(
    const bf16* __restrict__ partial, const bf16* __restrict__ gamma_q_a,
  ...
  const bool live = b < B;
  const bf16* __restrict__ P = partial + (long long)(live ? b : 0) * MLA_FUSED;

#pragma unroll
  for (int k = 0; k < 3; ++k) {
    const int u = 32 * (3 * sub + k) + lane;
    const float4 v = load4bf16(P + 4 * u);
