...
//
//   nvcc -cubin -arch=sm_103a -o k3_rs_arrive_v2.cubin k3_rs_arrive_v2.cu
    ...
    long count, int units, int slot, int order, int rank, int nranks)
{
    ...
    /* One thread per peer.  The four counters live in four different ranks, so
     * reading them one after the other costs four remote round trips in series;
     * issued from four threads they overlap.  The block is a single warp, so
     * __syncwarp() is the whole barrier, and every counter still has to reach
     * the epoch before the block (and therefore the kernel) is done.  Every
     * counter is far below 2^32, so the low half of the u64 is the whole value. */
    const int q = threadIdx.x;
    if (q < nr) {
        volatile const unsigned* f =
            (volatile const unsigned*)(flags_peer[q] + (unsigned long long)slot * 8u);
        const unsigned target = (unsigned)flags[0];
