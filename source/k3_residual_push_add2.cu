// res_in fused with the land_add2 that precedes it (round 25)
//
// `l<N>.hidden` (`kern_k3_land_add2_bf16`, module k3_land_bf16) writes
// `hidden = bf16(prefix2 + p1 (+ p2))` and `l<N+1>.res_in` -- the *next* call in
// the graph -- reads that row back as the prefix candidate of its mix.  The two
// are one row-wise expression apart: this module computes the row in registers,
// writes it out for the attention and the MoE, and uses it directly.
//
// res_in is wire-bound (176 MB of peer stores per call at the fabric rate, its
// local traffic at ~4.4 TB/s of the ~5.8 available), so the 176 MB of reads
// land_add2 needed fit under it; what disappears is land_add2's own 39.4 us per
// call plus one launch.  Everything else -- the candidate mix, the statistics
// hand-off of round 20, the four-rank push, the arrival -- is
// source/k3_residual_push_stats.cu unchanged.

// res_in also computes the MLP residual's candidate statistics (round 20)
...
__device__ __forceinline__ void attnres_rms_row(
    ...
    int rank, int nranks,
    int nb, int t,
    const float*  __restrict__ sw_mlp,    // [H] the MLP residual's scores vector
    float*        __restrict__ stats,     // [B, KNB_MAX, 2] (sq_c, dp_mlp_c)
    int row)
{
    const int  lane = t & 31, warp = t >> 5;

    __shared__ float s_red[(KNB_MAX + 1) * 2 * KNWARPS];   // [value][warp] partials
    __shared__ float s_redm[(KNB_MAX + 1) * KNWARPS];      // dp_mlp warp partials
    ...
    if (nb > 0) {
        ...
        float dpm[KNB_MAX + 1];
#pragma unroll
        for (int c = 0; c <= KNB_MAX; ++c) { sq[c] = 0.f; dp[c] = 0.f; dpm[c] = 0.f; }
#pragma unroll
        for (int v = 0; v < KVPT; ++v) {
            ...
            const float4* mp4 = (const float4*)(sw_mlp + u);
            const float4 ma = mp4[0], mb = mp4[1];
            const float swmv[8] = { ma.x, ma.y, ma.z, ma.w, mb.x, mb.y, mb.z, mb.w };
            V8 x[KNB_MAX];
#pragma unroll
...
#pragma unroll
                for (int j = 0; j < 4; ++j) {
                    ...
                    dpm[c] += f.x * swmv[2 * j];  dpm[c] += f.y * swmv[2 * j + 1];
                }
            }
        }
#pragma unroll
        for (int c = 0; c < KNB_MAX; ++c) {
            ...
            const float dm = warp_sum(dpm[c]);
            if (lane == 0) {
                s_red[(c * 2) * KNWARPS + warp] = s;
                s_red[(c * 2 + 1) * KNWARPS + warp] = d;
                s_redm[c * KNWARPS + warp] = dm;
            }
        }
        {
...
#pragma unroll
            ...
            /* the MLP residual's statistics ride along: sq is the same number
             * res_mlp would compute and dp only differs in which sw it uses, so
             * res_mlp does not have to read the candidates again.  The summation
             * order is res_mlp's own, so the value it reads is bit-identical. */
            if (lane < nb) {
                float dpmv = 0.f;
#pragma unroll
                for (int w = 0; w < KNWARPS; ++w) dpmv += s_redm[lane * KNWARPS + w];
                stats[((size_t)row * KNB_MAX + lane) * 2 + 0] = sqv;
                stats[((size_t)row * KNB_MAX + lane) * 2 + 1] = dpmv;
            }
        }
        float mx = sc;
#pragma unroll
...
extern "C" __global__ void __launch_bounds__(RES_THREADS, 2) kern_k3_attnres_rms_push(
    ...
    const float*  __restrict__ sw_mlp,    // [H] the MLP residual's scores vector
    float*        __restrict__ stats,     // [B, KNB_MAX, 2] out
    const bf16_t* __restrict__ p1,        // [B, H]  l*.hidden's three inputs
    const bf16_t* __restrict__ p2,        // [B, H]  (read only when two != 0)
    const bf16_t* __restrict__ prefix2,   // [B, H]
    bf16_t*       __restrict__ hidden,    // [B, H]  written, and used in registers
    int two)
{
    ...
    /* l*.hidden (`kern_k3_land_add2_bf16`) computed this row and wrote it to
     * global memory; this launch, the very next call, read it back.  The value
     * that lands is `bf16(prefix2 + p1 (+ p2))` -- the same expression, the same
     * rounding, in the same order as that kernel -- so folding the two launches
     * into one removes a 58.7 MB read and a launch per layer and touches no
     * arithmetic.  The row still reaches memory: the attention and the MoE read
     * `hidden` after this call. */
    V8 pv[KVPT];
#pragma unroll
    for (int v = 0; v < KVPT; ++v) {
        const int u = (t + v * RES_THREADS) * 8;
        const V8 f1 = ldv(p1 + (size_t)b * KH + u);
        const V8 fp = ldv(prefix2 + (size_t)b * KH + u);
        V8 f2;
        if (two) f2 = ldv(p2 + (size_t)b * KH + u);
        V8 o;
#pragma unroll
        for (int j = 0; j < 4; ++j) {
            const float2 a = bf2f(fp.w[j]);
            const float2 c = bf2f(f1.w[j]);
            float x = a.x + c.x, y = a.y + c.y;
            if (two) {
                const float2 e = bf2f(f2.w[j]);
                x += e.x; y += e.y;
            }
            o.w[j] = f2bf(make_float2(x, y));
        }
        pv[v] = o;
        stv(hidden + (size_t)b * KH + u, o);
    }
...
#pragma unroll
    ...
    attnres_rms_row(blocks + (size_t)b * KNB_MAX * KH, pv, sw, gamma,
                    dst_row, dst_peer, dst_off, rank, nr, nb, t, sw_mlp, stats, b);
