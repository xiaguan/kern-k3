// `k3_moe_prefill_v3.cu` with `kern_k3_moe_route_tables` restructured, same ABI and
// same outputs (bit-identical, the probe in `round20-route-fold/` compares all six
// buffers):
//
//   * the counts are summed first and the expert offset is folded into the single
//     write loop, so `blockoff` is written once instead of read-modify-written --
//     the round-19 probe measured the shipped two-phase version at 11.18 us and
//     this one at 9.06;
//   * the tile is a compile-time constant (`-DTILE_SHIFT=7` for tile 128), so the
//     scan's `ceil(count/tile)` is a shift instead of the software integer
//     division the task book calls out; `tile` is still passed and is asserted to
//     agree at build time by the generator that wires it up.
//
// k3_moe_prefill_v2.cu with one change: with -DEP_STRIDED=1 the routed experts
// are placed on ranks round robin (rank r owns the global ids e with
// e % EP_RANKS == r, local slot e / EP_RANKS) instead of in blocks of E, so
// kern_k3_moe_route_count and kern_k3_moe_route_scatter take the local expert
// id from that map. Every other entry and every arithmetic operation is
// identical to v2; the manifest's weight binds are reordered to match.
//
...
/* Warps in the scatter's block.  ROUTE_GROUPS/(32*SCATTER_WARPS) rounds cover a
   token block; the manifest's block must be 32*this.  The staged work is
   identical, only how many warps share it. */
#ifndef SCATTER_WARPS
#define SCATTER_WARPS 8
#endif

// Local expert slot of a global expert id, or -1 when another rank owns it.
// The manifest's weight binds must agree with this map.
__device__ __forceinline__ int local_expert(int id, int rank, int E) {
#ifdef EP_STRIDED
  if (id % EP_RANKS != rank) return -1;
  const int e = id / EP_RANKS;
  return e < E ? e : -1;
#else
  const int e = id - rank * E;
  return (e >= 0 && e < E) ? e : -1;
#endif
}
...
#pragma unroll
    for (int k = 0; k < TOPK; ++k) {
      const int e = local_expert(ids[t * TOPK + k], rank, E);
...
extern "C" __global__ void kern_k3_moe_route_tables(const int* __restrict__ blockcount, int blocks, int E, int tile,
  ...
  if (e < E) {
    int run = 0;
    for (int b = 0; b < blocks; ++b) run += blockcount[b * E + e];
    count[e] = run;
  }
  __syncthreads();
  if (threadIdx.x == 0) {
    ...
    for (int j = 0; j < E; ++j) {
      off[j] = run;
#ifdef TILE_SHIFT
      run += (count[j] + (1 << TILE_SHIFT) - 1) >> TILE_SHIFT;
#else
  ...
  __syncthreads();
  if (e < E) {
    /* one write per (block, expert): the running sum is recomputed from the
       block counts instead of read-modify-writing the array the first loop left */
    int run = off[e] * tile;
    for (int b = 0; b < blocks; ++b) {
      blockoff[b * E + e] = run;
      run += blockcount[b * E + e];
    }
...
extern "C" __global__ void __launch_bounds__(32 * SCATTER_WARPS) kern_k3_moe_route_scatter(const int* __restrict__ ids, const int* __restrict__ blockoff,
  ...
  unsigned peers[TOPK];
#pragma unroll
  for (int r = 0; r < (ROUTE_GROUPS + SCATTER_WARPS - 1) / SCATTER_WARPS; ++r) {
    const int g = r * SCATTER_WARPS + warp;
    if (g >= ROUTE_GROUPS) break;
    const long long idx = base + g * 32 + lane;
    int e = -1;
    if (idx < (long long)T * TOPK) e = local_expert(ids[idx], rank, E);
    ex[r] = e;
