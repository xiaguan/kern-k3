...
2. 修改 kernel source 或调用方式，生成独立 candidate JSON 和 cubin。保留旧 cubin，使 reference 仍可运行。允许改变中间 dtype、归约顺序、线程布局和融合方式，以 kern test 原有阈值通过为准；不额外要求 bitwise / 逐位相等。不要为了保持逐位一致而限制优化空间，也不得放宽测试阈值。
...
不要修改评测入口、workload、runtime、模型权重、reference、测试阈值，不要删除算子或针对固定输入硬编码结果。不能用 eager 时间代替 graph 时间。遇到其他任务占 GPU、测试失败或环境异常，先记录原因，不发布收益结论。不得启动子 agent。GPU 被其他任务占用时，停止包括微基准在内的 GPU 测试，等待独占资源。
...
迁移到其他 GPU 节点后必须重新测 baseline，再在同一节点交替测候选；不能把不同节点的成绩直接比较。当前 residual 候选和未提交 source 改动是上一轮保留的进度，先阅读实验 README 并继续评估。核对 manifest 实际参数分布；nb=1 只覆盖部分 residual 调用，不得把该形状的微基准收益当成全部调用的收益。
