#!/usr/bin/env bash
set -euo pipefail
: "${BASE_IMAGE:?}" "${KERN_BINARY:?}" "${NCCL_LIB_DIR:?}" "${WEIGHTS:?}" "${TOKENIZER:?}" "${CODEX_AUTH_FILE:?}"
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
  echo 'GPUs have active compute processes; wait for an idle allocation.' >&2
  exit 1
fi
codex_binary="$(readlink -f "$(command -v codex)")"
...
docker run -d --name "$container" --gpus all --ipc=host --ulimit memlock=-1:-1 \
  ...
  --mount "type=bind,src=$codex_binary,dst=/usr/local/bin/codex,readonly" \
  --mount "type=bind,src=$(dirname "$codex_binary")/codex-code-mode-host,dst=/usr/local/bin/codex-code-mode-host,readonly" \
  --mount "type=bind,src=$CODEX_AUTH_FILE,dst=/run/codex-auth.json,readonly" \
  ...
  "$image"
docker exec "$container" sh -c 'mkdir -p "$HOME/.codex" && cp /run/codex-auth.json "$HOME/.codex/auth.json" && chmod 600 "$HOME/.codex/auth.json"'
