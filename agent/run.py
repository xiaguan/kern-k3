...
def main():
    ...
    os.chdir(root)
    agent = KimiCodeCLIAgent(KimiCodeCLIAgentConfig(model="kimi-code/k3-256k", effort="max", goals=False))
    runner = Runner(root / "agent/flow.py", [agent], {"rounds": args.rounds})
    if args.check:
        print("Valid: container Kimi Code, kimi-code/k3-256k, max; no model request sent")
        return
